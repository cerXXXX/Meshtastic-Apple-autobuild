...
- [x] T042 [P] Dynamic Type clipping in compact widgets — `DistanceCompactWidget.swift`, `HumidityCompactWidget.swift` (and siblings) capped `maxHeight: 140` while using `.font(.largeTitle)` inside; text clipped at larger accessibility sizes. Fixed by dropping the `maxHeight`/`idealHeight` cap so the container can grow with the scalable header/unit labels.
